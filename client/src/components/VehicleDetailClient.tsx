...
export default function VehicleDetailClient({ initialCar }: Props) {
    ...
    useEffect(() => {
        if (initialCar) {
            dispatch({ type: "SET_SELECTED_CAR_ID", payload: initialCar.id.toString() });
    ...
    const car = state.cars.find((c) => c.id.toString() === initialCar.id.toString()) ?? initialCar;

    return (
        <main>
            <h1>{`${car.make} ${car.model} ${car.variant ?? ""}`}</h1>
            <ul>
                <li><strong>VRM:</strong> {car.vrm}</li>
                ...
                <li><strong>Price:</strong> £{car.activeAdvertPriceAmount}</li>
            </ul>
        </main>
    );
}
